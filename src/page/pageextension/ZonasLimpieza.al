pageextension 92173 ZonasLimpiezaExt extends "Zonas Limpieza"
{
    ...
    actions
    {
        ...
        addlast(Processing)
        {
            ...
            action(ImportarParadasRevision)
            {
                ApplicationArea = All;
                Caption = 'Importar paradas revisión';
                Image = Import;
                ToolTip = 'Importa un archivo Excel con números de emplazamiento en la columna A y asigna la zona de limpieza actual a cada uno.';

                trigger OnAction()
                begin
                    ImportarParadasRevisionDesdeExcel(Rec);
                end;
            }
        }
    }
    ...
    local procedure ImportarParadasRevisionDesdeExcel(var ZonaLimpiezaActual: Record "Zonas Limpieza")
    var
        FileMgt: Codeunit "File Management";
        TempBlob: Codeunit "Temp Blob";
        ExcelBuffer: Record "Excel Buffer" temporary;
    ...
    begin
        ...
        if ExcelBuffer.FindSet() then
            repeat
                ...
                if NumEmplazamiento <> '' then begin
                    if Emplazamientos.Get(Emplazamientos."Tipo Emplazamiento"::Opis, NumEmplazamiento) then begin
                        Emplazamientos.Validate("Zona Revisión", ZonaLimpiezaActual.Id);
                        Emplazamientos.Modify(true);
                        Actualizados += 1;
                    end else
                        NoEncontrados += 1;
